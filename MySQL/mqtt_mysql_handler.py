...
engine = create_engine("mysql+mysqldb://"+ config.mysql_user + ":" + config.mysql_password + "@" + config.mysql_host + "/watering_server?host="+ config.mysql_host + "?port=" + config.port, pool_recycle=3600, pool_pre_ping=True)
conn = engine.connect()
...
owm = pyowm.OWM(config.owm_api) #openweathermap
